...
export class CampoService{
  ...
  public update(campo_to_update,user):Observable<any>{
    let params= JSON.stringify(campo_to_update);
    let headers = new HttpHeaders().set('Content-Type','application/json');
    headers = headers.append('Authorization',this.getToken());
    return this._http.put(this.url+'campos/updateCampo/'+campo_to_update._id+'/'+user, params, {headers: headers}).pipe(map(res => res));
  }
  ...
  public getCampo(idCampo):Observable<any>{
    ...
    let headers = new HttpHeaders().set('Content-Type','application/json');
    headers = headers.append('Authorization',this.getToken());
    return this._http.post(this.url+'campos/getCampo', params, {headers: headers}).pipe(map(res => res));
  }
